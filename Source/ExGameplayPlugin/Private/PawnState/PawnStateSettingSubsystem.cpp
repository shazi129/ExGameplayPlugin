...
void UPawnStateSettingSubsystem::RegisterLevelChangeHandler()
{
	...
	for (const FWorldContext& WorldContext : WorldContextList)
	{
		...
		{
			EXGAMEPLAY_LOG(Log, TEXT("%s handle already exist world[%s]"), *FString(__FUNCTION__), *World->GetPackage()->GetName());
...
void UPawnStateSettingSubsystem::OnPostWorldInit(UWorld* World, const UWorld::InitializationValues IVS)
{
	...
	EXGAMEPLAY_LOG(Log, TEXT("%s handle post world init [%s]"), *FString(__FUNCTION__), *WorldName);
...
void UPawnStateSettingSubsystem::OnWorldTearingDown(UWorld* World)
{
	...
	FString WorldName = *FPackageName::GetShortName(World->GetPackage()->GetName());
	EXGAMEPLAY_LOG(Log, TEXT("%s handle tearing down world[%s]"), *FString(__FUNCTION__), *WorldName);
...
void UPawnStateSettingSubsystem::OnLevelAdded(ULevel* Level, UWorld* World)
{
	if (!World || !Level)
	{
		EXGAMEPLAY_LOG(Log, TEXT("%s World Or Level is null"), *FString(__FUNCTION__));
		return;
	}
	...
	if (PawnState)
	{
		EXGAMEPLAY_LOG(Log, TEXT("%s World[%s] Level[%s] PawnState[%s]"), *FString(__FUNCTION__), *WorldName, *LevelName, *PawnState->ToString());
		ACharacter* Character = UGameplayStatics::GetPlayerCharacter(World, 0);
		UPawnStateLibrary::EnterPawnState(Character, FPawnStateInstance(PawnState, Level, nullptr));
	}
	else
	{
		EXGAMEPLAY_LOG(Log, TEXT("%s World[%s] Level[%s] PawnState[null]"), *FString(__FUNCTION__), *WorldName, *LevelName);
	}
}
...
const UPawnStateAsset* UPawnStateSettingSubsystem::GetGlobalPawnStateAsset(FGameplayTag PawnStateTag)
{
	...
	{
		EXGAMEPLAY_LOG(Error, TEXT("%s error, Invalid PawnState Tag"), *FString(__FUNCTION__));
		return nullptr;
	}

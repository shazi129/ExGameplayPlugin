...
UUIPopLayout* UUIPanel::ShowPopLayout(TSubclassOf<UUserWidget> PopClass, UPARAM(ref) FPopLayoutParam& PopParam)
{
	if (PopClass == nullptr)
	{
		UIMANAGER_LOG(Warning, TEXT("---UUIPanel ShowPopLayout Error, Pop Class is NULL"));
		return nullptr;
	}
	...
	{
		UIMANAGER_LOG(Warning, TEXT("---UUIPanel ShowPopLayout Error, Tempalte[%s] has no UIPoplayout Layer"), *this->GetName());
		return nullptr;
	}
	...
	if (PopTemplateClass == nullptr)
	{
		...
		{
			UIMANAGER_LOG(Warning, TEXT("---UUIPanel ShowPopLayout Error, Pop Template Class is NULL"));
			return nullptr;
		}
	}
	...
	if (Widget == nullptr)
	{
		UIMANAGER_LOG(Warning, TEXT("---UUIPanel ShowPopLayout Error, Create PopLayout Widget return NULL"));
		return nullptr;
	}
	...
	{
		UIMANAGER_LOG(Warning, TEXT("---UUIPanel ShowPopLayout Error, PopTemplateClass is not a UUIPopLayout"));
		return nullptr;
	}
...
void UUIPanel::RemoveAllPop()
{
	...
	if (LayoutPanel == nullptr)
	{
		UIMANAGER_LOG(Warning, TEXT("---UUIPanel RemoveAllPop Error, Tempalte[%s] has no UIPoplayout Layer"), *this->GetName());
		return;
	}
